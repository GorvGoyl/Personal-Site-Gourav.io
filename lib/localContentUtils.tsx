...
export function getPostBySlug(slug: string, fields: string[], postsDirectory: string): FrontmatterProject {
    const pathToPost = join(postsDirectory, slug);
    const files = fs.readdirSync(pathToPost);
    const indexFile = files.find((file) => {
        return file.substr(0, file.lastIndexOf('.')) === 'index';
    });

    if (!indexFile) {
        throw new Error('index file not found for slug: ' + slug);
    }
    // const ogImg =
...
export async function getPost(
    ...
    const result = await bundleMDX({
        ...
        cwd: mdDir,
        mdxOptions: (options) => {
            options.remarkPlugins = [...(options.remarkPlugins ?? []), remarkGfm, remarkMdxImages] as any;
